...
func TestAuthFlow(t *testing.T) {
	...
	resp, body = c.do("GET", "/v1/me", nil)
	mustStatus(t, resp, body, http.StatusOK)

	// Unauthenticated /me -> 401.
	anon := newClient(t, srv.URL)
	resp, body = anon.do("GET", "/v1/me", nil)
	mustStatus(t, resp, body, http.StatusUnauthorized)

	// Refresh via cookie -> 200 with a new access token.
	c.token = ""
	resp, body = c.do("POST", "/v1/auth/refresh", nil)
	mustStatus(t, resp, body, http.StatusOK)
	c.token = accessTokenFrom(t, body)

	// Login -> 200.
	resp, body = c.do("POST", "/v1/auth/login", map[string]string{
...
func TestSignupValidation(t *testing.T) {
	...
	for _, payload := range cases {
		resp, body := c.do("POST", "/v1/auth/signup", payload)
...
func TestOAuthDisabledReturns404(t *testing.T) {
	...
	resp, body := c.do("GET", "/v1/auth/oauth/google", nil)
